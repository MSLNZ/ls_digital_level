...
class App(tk.Tk):
    def __init__(self):
        ...
        # logging file names
        self.log_name = os.path.join(os.path.join(os.environ['USERPROFILE']), 
            'Desktop', 
            'LS15_' + 
            '{:%Y%m%d_%H%M}'.format(datetime.datetime.now()) + 
            '.log')
        
        self.csv_name = os.path.join(os.path.join(os.environ['USERPROFILE']), 
            'Desktop', 
            'LS15_' + 
            '{:%Y%m%d_%H%M}'.format(datetime.datetime.now()) + 
            '.csv')
    ...
    def process_serial(self):
        while self.queue.qsize():
            try:
                ...
                log.write('{} {}'.format(datestamp, str(s)))
    ...
    def send_to_excel(self):
        ...
        if self.data_excel.get() == 'All':
            ...
            self.excel.ActiveCell.Offset(*offset_B).Value = self.distance_var.get()
            self.excel.ActiveCell.Offset(*offset_C).Value = self.angle_var.get()
            self.excel.ActiveCell.Offset(*offset_D).Value = '{:.5f}'.format(self.X)
            self.excel.ActiveCell.Offset(*offset_E).Value = '{:.5f}'.format(self.Y)
            self.excel.ActiveCell.Offset(*offset_F).Value = '{:.5f}'.format(self.Z)
            self.excel.ActiveCell.Offset(*offset_A).Select()
    ...
    def send_to_clipboard(self):
        ...
        if self.data_clip.get() == 'Height, Distance, Bearing':
            self.clipboard_append('{}, {}, {}'.format(self.height_var.get(), 
                                                      self.distance_var.get(), 
                                                      self.angle_var.get()))

        if self.data_clip.get() == 'X, Y, Z':
            self.calc_XYZ()
            self.clipboard_append('{:.5f}, {:.5f}, {:.5f}'.format(self.X, self.Y, self.Z))
        
        if self.data_clip.get() == 'All':
            self.calc_XYZ()
            self.clipboard_append('{}, {}, {}, {:.5f}, {:.5f}, {:.5f}'.format(self.height_var.get(), 
                                                      self.distance_var.get(), 
                                                      self.angle_var.get(),
    ...
    def create_display(self):
        ...
        #-----------------Serial Port column-----------------------------------                
        self.open_serial_btn = tk.Button(self, text="Open Serial Port", command=self.open_serial)
        ...
        self.close_serial_btn = tk.Button(self, text="Close Serial Port", command=self.close_serial)
        self.close_serial_btn.grid(row=25, column=0, sticky='W')
        
        #----------------Excel column-----------------------------------
        
        self.open_workbook_btn = tk.Button(self, text="Open Excel Workbook", command=self.open_workbook)
